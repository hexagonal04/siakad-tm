...
class Data_staffkeu extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->sespre = $this->config->item('session_name_prefix');

        $this->d['admlevel'] = $this->session->userdata($this->sespre.'level');
        $this->d['url'] = "data_staffkeu";
        $this->d['idnya'] = "datastaffkeu";
        $this->d['nama_form'] = "f_datastaffkeu";
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM m_keuangan")->num_rows();
    
        $qdata = $this->db->query("SELECT 
                                        a.*,
                                        (SELECT COUNT(id) FROM m_admin WHERE level = 'keuangan' AND konid = a.id) AS jml_aktif,
                                        b.username
                                        FROM m_keuangan a
                                        LEFT JOIN m_admin b ON CONCAT('keuangan',a.id) = CONCAT(b.level,b.konid) 
    ...
    public function edit($id) {
        $q = $this->db->query("SELECT *, 'edit' AS mode FROM m_keuangan WHERE id = '$id'")->row_array();
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO m_keuangan (nama, jk) VALUES ('".$p['nama']."','".$p['jk']."')");

            $d['status'] = "ok";
            $d['data'] = "Data berhasil disimpan";
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE m_keuangan SET nama = '".$p['nama']."', jk = '".$p['jk']."' WHERE id = '".$p['_id']."'");
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM m_keuangan WHERE id = '$id'");
    ...
    public function aktifkan($id) {
        ...
        $detil_data = $this->db->query("SELECT nama FROM m_keuangan WHERE id = '".$id."'")->row_array();
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("INSERT INTO m_admin (username,password,level,konid,aktif) VALUES ('".$username_fix."', '".$password."', 'keuangan', '$id', 'Y')");
    ...
    public function nonaktifkan($id) {
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("DELETE FROM m_admin WHERE level = 'keuangan' AND konid = '$id'");
