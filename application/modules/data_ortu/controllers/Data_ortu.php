...
class Data_ortu extends CI_Controller {
	function __construct() {
        ...
        $this->d['url'] = "data_ortu";
        $this->d['idnya'] = "dataortu";
        $this->d['nama_form'] = "f_dataortu";
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM m_ortu_siswa")->num_rows();
    
        $qdata = $this->db->query("SELECT 
                                        a.*,
                                        (SELECT COUNT(id) FROM m_admin WHERE level = 'ortu' AND konid = a.id) AS jml_aktif,
                                        b.username
                                        FROM m_ortu_siswa a
                                        LEFT JOIN m_admin b ON CONCAT('ortu',a.id) = CONCAT(b.level,b.konid) 
    ...
    public function edit($id) {
        $q = $this->db->query("SELECT *, 'edit' AS mode FROM m_ortu_siswa WHERE id = '$id'")->row_array();
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO m_ortu_siswa (nama, jk, id_siswa) VALUES ('".$p['nama']."','".$p['jk']."','".$p['id_siswa']."')");

            $d['status'] = "ok";
            $d['data'] = "Data berhasil disimpan";
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE m_ortu_siswa SET nama = '".$p['nama']."', jk = '".$p['jk']."', id_siswa = '".$p['id_siswa']."' WHERE id = '".$p['_id']."'");
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM m_ortu_siswa WHERE id = '$id'");
    ...
    public function aktifkan($id) {
        ...
        $detil_data = $this->db->query("SELECT nama FROM m_ortu_siswa WHERE id = '".$id."'")->row_array();
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("INSERT INTO m_admin (username,password,level,konid,aktif) VALUES ('".$username_fix."', '".$password."', 'ortu', '$id', 'Y')");
    ...
    public function nonaktifkan($id) {
        ...
        if (empty($detil_data)) {
            ...
            $this->db->query("DELETE FROM m_admin WHERE level = 'ortu' AND konid = '$id'");
